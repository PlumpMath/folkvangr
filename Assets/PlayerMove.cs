    ...
    void Update()
    {
        ...
        if (!repositioning)
        {
            if (!MessageBox.displaying)
            {
                ...
                if (!recall)
                {
                    xvel = Input.GetAxis("Horizontal") * 5f;
                ...
                else
                {
                    if (GameObject.FindGameObjectWithTag("Mask") && GameObject.FindGameObjectWithTag("Mask").GetComponent<MaskSpin>())
                    {
                        ...
                        transform.position = Vector2.Lerp(transform.position, msk.transform.position, Time.fixedDeltaTime * 8f * (flytime < 0.5f ? 1f : 2 * flytime * flytime));
