...
public class MaskSpin : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        target = transform.position + new Vector3(dx * PlayerStats.Speed, Input.GetAxisRaw("Vertical") * PlayerStats.Speed, 0);
                ...
                for (int i = 0; i < rh.Length; i++)
                {
                    for (int j = 0; j < rh[i].Length; j++)
                    {
                        ...
                        if (rh[i][j].collider.transform.tag != "Player")
                        {
                            ...
                            {
                                rh[i][j].collider.transform.gameObject.GetComponent<MonsterStats>().Hit(PlayerStats.Attack);
                            }
                            else if (rh[i][j].collider.transform.tag == "Card" && rh[i][j].collider.transform.gameObject.GetComponent<TuningCard>())
                            {
                                stuck = false;
                                recall = true;
